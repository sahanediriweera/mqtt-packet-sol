...
struct mqtt_ack *mqtt_packet_ack(unsigned char byte,unsigned char pkt_id){
    static struct mqtt_ack ack;
    ack.header.byte = byte;
    ack.pkt_id = pkt_id;
    return &ack;
...
struct mqtt_connack *mqtt_packet_conack(unsigned char byte, unsigned char cflags, unsigned char rc){
...
struct mqtt_suback *mqtt_packet_suback(unsigned char byte,unsigned short pkt_id, unsigned char *rcs,unsigned short rcslen){
    struct mqtt_suback *suback = (mqtt_suback*) malloc(sizeof(mqtt_suback));
    suback->header.byte = byte;
    suback->pkt_id = pkt_id;
    suback->rcslen = rcslen;
    suback->rcs = (unsigned char *) malloc(rcslen);
...
static unsigned char *pack_mqtt_publish(const union mqtt_packet *pkt){
    //Total length must me calculated = header and length of the fixed header part
    size_t pktlen = MQTT_HEADER_LEN + sizeof(uint16_t)+pkt->publish.topiclen+ pkt->publish.payloadlen;
    //Total length excluding the fixed header len
    size_t len = 0L;
    if(pkt->header.bits.qos > AT_MOST_ONCE) pktlen += sizeof(uint16_t);
    int remaininglen_offset = 0;
    if((pktlen-1)> 0x200000) remaininglen_offset = 3;
    else if((pktlen-1)>0x4000) remaininglen_offset = 2;
    else if((pktlen-1)>0x80) remaininglen_offset = 1;
    pktlen += remaininglen_offset;

    unsigned char *packed = (unsigned char *) malloc(pktlen);
    unsigned char *ptr = packed;
    ...
    if(pkt->header.bits.qos > AT_MOST_ONCE) pack_u16(&ptr, pkt->publish.pkt_id);
